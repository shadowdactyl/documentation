...
## Creating a database for Shadowdactyl
MySQL is a core component of Shadowdactyl Panel but it can be confusing to setup and use if you've never done so before.
...
Next, we will create a user called `shadowdactyl` and allow logins from localhost which prevents any external connections
...
CREATE USER 'shadowdactyl'@'127.0.0.1' IDENTIFIED BY 'somePassword';
```
...
Finally, we need to tell MySQL that our shadowdactyl user should have access to the panel database. To do this, simply
run the command below.

``` sql
GRANT ALL PRIVILEGES ON panel.* TO 'shadowdactyl'@'127.0.0.1';
```
...
CREATE USER 'shadowdactyluser'@'127.0.0.1' IDENTIFIED BY 'somepassword';
```
...
```sql
GRANT ALL PRIVILEGES ON *.* TO 'shadowdactyluser'@'127.0.0.1' WITH GRANT OPTION;
```
