...
``` bash
curl -L https://github.com/shadowdactyl/daemon/releases/download/v0.5.7/daemon.tar.gz | tar --strip-components=1 -xzv
```
...
``` bash
docker rmi -f quay.io/shadowdactyl/scrappy
...
If for whatever reason you need your user to be named something other than `shadowdactyl` you can change that in the
